...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active"><a href="myfiles.php" style="color:#fff; font-size:medium"><i class="fa fa-archive"></i> Mis Archivos</a></li>
            <?php
                    if(@mysqli_num_rows($folder)!=0){
                        echo '<li class="active"><a href="myfiles.php?folder='.$file_folder_code.'" style="color:#fff; font-size:medium"><i class="fa fa-folder-open"></i> '.$file_folder_filename.'</a></li>';
                    }
                ?>
        </ol>
    </section>
    ...
    <section class="content">
                ...
                <?php
                    ...
                    if(@mysqli_num_rows($folder)==0){
                        if(isset($_GET["q"]) && $_GET["q"]!=""){
                            $q=$_GET["q"];
                            $files = mysqli_query($con,"select * from file where usuario='$my_user_id' and folder_id is NULL and (filename like '%$q%' or description like '%$q%') order by is_folder desc, filename asc");
                           
                    }else{
                        $files = mysqli_query($con,"select * from file where usuario='$my_user_id' and folder_id is NULL order by is_folder desc, filename asc");
                    }
        ...
        //by abisoft
        $url = "storage/data/".$file['usuario']."/".$file['filename'];
        $ftype=explode(".", $url);
